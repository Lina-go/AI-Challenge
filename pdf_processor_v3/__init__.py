...
def process_document_with_signatures(
    pdf_path: str,
    output_path: str = None,
    llm_provider: str = "openai",
    llm_model: str = None,
    **kwargs
    ...
    """
    Interfaz simple para procesar un documento PDF con extracción de signatures.
    Modo híbrido que extrae texto, tablas, figuras Y datos de signatures.
    ...
    Returns:
        DocumentResult object with signature data included
    """
    # Create configuration with signature extraction enabled
    config = ProcessorConfig.create_preset(
        llm_provider if llm_provider in ["openai", "anthropic"] else "openai"
    )
    ...
    # ACTIVAR LA NUEVA FUNCIONALIDAD
    config.task.extract_signatures_in_document_mode = True
